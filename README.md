...
4. **Accessing the Environment:**
    * After apply, Terraform outputs RDP and HTTPS connection strings directly in the terminal.
    * A `connection_info.md` file is generated in the project directory with all connection details formatted as a Markdown table — ready to share with demo participants.
...
* `providers.tf`: Defines required providers (AzureRM, Time, Local) and the Azure backend for remote state.
...
* `outputs.tf`: Public IPs, private IPs (PDC, RCB, WTS, SGW), subnet IDs, direct RDP connection string, HTTPS URL, and the `local_file` resource that generates `connection_info.md`.
* `terraform.tfvars`: Local variable values (keep this file secure — contains the admin password).
* `connection_info.md`: **Auto-generated by `terraform apply`** — contains all connection details (IPs, RDP link, HTTPS URL, credentials hint). Excluded from version control via `.gitignore`.
...
* **Security:** The Jump Host and Gateway have Public IPs. Use strong passwords and consider restricting source IP ranges in the NSG rules for production-like scenarios. The `terraform.tfvars` file contains the admin password and must not be committed to version control. The auto-generated `connection_info.md` contains live IP addresses and is also excluded from version control.
